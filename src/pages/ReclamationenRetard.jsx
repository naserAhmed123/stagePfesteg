...
export default function ReclamationsPage() {
  ...
  const fetchReclamations = async () => {
    ...
    try {
      ...
      // Map and filter reclamations for current day
      const today = moment().startOf('day');
      const mappedReclamations = response.data
        .map((reclamation) => ({
          id: reclamation.id,
        ...
        .filter((reclamation) => {
          const time = moment(reclamation.stateChangeTime);
          return time.isSame(today, 'day'); // Only current day
        });
  ...
  const downloadPDF = () => {
    try {
      ...
      doc.setFontSize(14);
      doc.text('Réclamations en Cours Retardées (Jour Actuel) - Rapport', 10, 20);
      ...
      autoTable(doc, {
        ...
        theme: 'striped',
        headStyles: { fillColor: [59, 130, 246], textColor: 'white' },
        styles: { fontSize: 10 },
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-16">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-4 sm:mb-0">
            Réclamations <span className="text-blue-600">Retardées du Jour</span>
          </h1>
          <div className="flex items-center space-x-4">
            <div className="relative">
              <input
                ...
                className="w-full sm:w-64 pl-10 pr-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
                onChange={(e) => handleSearch(e.target.value)}
                aria-label="Search reclamations"
              />
              <svg
            ...
            <button
              onClick={downloadPDF}
              className="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg transition-all duration-200 flex items-center"
        ...
        {!loading && !error && paginatedReclamations.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-500">
              Aucune réclamation retardée du jour actuel trouvée
            </p>
          </div>
        )}
                  ...
                  {paginatedReclamations.map((reclamation) => (
                    <tr key={reclamation.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${getImportanceColor(
            ...
            <div className="bg-gray-50 px-6 py-4 flex justify-between items-center">
              ...
              <div className="flex space-x-2">
                <button
                  ...
                  className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 disabled:bg-gray-200 disabled:cursor-not-allowed transition-all"
