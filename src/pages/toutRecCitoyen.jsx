...
const ViewAllReclamations = () => {
  ...
  const [sortConfig, setSortConfig] = useState({ key: 'heureReclamation', direction: 'desc' });
  ...
  // Etat progress mapping
  const etatProgress = {
    PAS_ENCOURS: { progress: 0, color: 'bg-gray-200', text: 'Non commencé' },
    ENCOURS: { progress: 33, color: 'bg-blue-500', text: 'En cours' },
    PROBLEM: { progress: 66, color: 'bg-red-500', text: 'Problème' },
    TERMINER: { progress: 100, color: 'bg-green-500', text: 'Terminé' },
    ANNULER: { progress: 0, color: 'bg-gray-400', text: 'Annulé' },
  };
  ...
  useEffect(() => {
    ...
    const filtered = reclamations.filter((reclamation) =>
      [
        ...
        reclamation.etat,
        reclamation.rue,
      ].some((field) => String(field).toLowerCase().includes(lowerCaseQuery))
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/reclamations/${editReclamation.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
      ...
      setFilteredReclamations(filteredReclamations.map((rec) => (rec.id === editReclamation.id ? { ...rec, ...payload } : rec)));
      showCustomAlert('Réclamation modifiée avec succès !', 'success');
      toggleModal();
    } catch (e) {
      console.error('Erreur lors de la modification de la réclamation:', e);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setFilteredReclamations(filteredReclamations.filter((rec) => rec.id !== id));
  ...
  const handleSort = (key) => {
    ...
    setFilteredReclamations([...filteredReclamations].sort((a, b) => {
      let aValue = a[key];
      let bValue = b[key];

      if (key === 'heureReclamation') {
  ...
  const CustomAlert = () => {
    ...
    return (
      <motion.div
        className="fixed top-5 right-5 z-[100] w-auto"
  ...
  return (
    <>
      <PageBreadcrumb pageTitle="Toutes Vos Réclamations" />
      <ToastContainer />
      <CustomAlert />
      <ConfirmModal />
      ...
      >
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-6">
          <h2 className="text-2xl font-semibold text-gray-800 dark:text-white mb-4">Gestion des Réclamations</h2>
          <p className="text-gray-600 dark:text-gray-300">
            Visualisez, modifiez ou supprimez toutes vos réclamations dans le tableau ci-dessous.
          </p>
          {/* Search Bar */}
          <div className="mt-4 relative">
            <input
              type="text"
              placeholder="Rechercher par référence, numéro client, type, genre, importance, état ou rue..."
              className="w-full pl-10 pr-4 py-2 rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
              value={searchQuery}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  {['reference', 'numClient', 'typePanne', 'genrePanne', 'importance', 'etat', 'rue', 'heureReclamation', 'Position2Km', 'etatSauvgarder'].map((key) => (
                    <th
                      key={key}
                      scope="col"
                    ...
                    >
                      <div className="flex items-center">
                        {key === 'heureReclamation' ? 'Date Réclamation' : 
                         key === 'Position2Km' ? 'Position' : 
                         key === 'etatSauvgarder' ? 'État Sauvgarde' : 
                         key === 'etat' ? 'État Progression' :
                  ...
                  currentItems.map((reclamation) => (
                    <tr key={reclamation.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span className={`px-2 py-1 rounded-full border ${importanceColors[reclamation.importance] || 'bg-gray-100 text-gray-800'}`}>
                          {reclamation.importance}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-300">
                        <div className="flex flex-col gap-2">
                          <div className="text-sm font-medium">{etatProgress[reclamation.etat]?.text}</div>
                          <div className="w-full bg-gray-200 rounded-full h-2.5">
                            <div
                              className={`h-2.5 rounded-full ${etatProgress[reclamation.etat]?.color}`}
                              style={{ width: `${etatProgress[reclamation.etat]?.progress}%` }}
                            ></div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{reclamation.rue}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{formatDate(reclamation.heureReclamation)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{reclamation.Position2Km}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{reclamation.etatSauvgarder}</td>
          ...
          {filteredReclamations.length > itemsPerPage && (
            <div className="px-6 py-4 flex justify-between items-center bg-gray-50 dark:bg-gray-700">
              <div className="text-sm text-gray-700 dark:text-gray-300">
                Affichage de {indexOfFirstItem + 1} à {Math.min(indexOfLastItem, filteredReclamations.length)} sur {filteredReclamations.length} réclamations
              </div>
              <div className="flex space-x-2">
                <motion.button
                  onClick={() => paginate(currentPage - 1)}
                  disabled={currentPage === 1}
                  className="px-3 py-1 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200 rounded disabled:opacity-50"
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
                  Précédent
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((number) => (
                  <motion.button
                    ...
                    className={`px-3 py-1 rounded ${currentPage === number ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200'}`}
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                  >
                    {number}
        ...
        >
          <div className="relative w-full max-w-2xl mx-4 bg-white dark:bg-gray-800 rounded-xl shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Modifier la Réclamation</h3>
              <motion.button
                onClick={toggleModal}
...
export default ViewAllReclamations;